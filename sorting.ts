...
const getIterator = async <T>(
  iterable: AnyIterable<T>
): Promise<StatefulIterator<T>> => {
  ...
  if (Symbol.asyncIterator in iterable) {
    ...
    const result = {
      current: await asyncIterator.next(),
      advance: async () => {
        result.current = await asyncIterator.next();
      },
    };
    return result;
  }
...
export const compareOn: <T>(key: (arg: T) => unknown) => Comparer<T> =
  (key) => (x, y) => {
    const k1: any = key(x);
    const k2: any = key(y);
...
export async function* merge<T>(
  ...
  const iteratorComparer: Comparer<StatefulIterator<T>> = (x, y) =>
    comparer(x.current.value, y.current.value);
  const sortedIterators = getSortedCollection(
    nonEmptyIterators,
    iteratorComparer
  );
...
async function* getSortedChunks<T>(
  iterable: AnyIterable<T>,
  { maxSize, comparer, store }: Options<T>
) {
...
async function aggregateChunks<T>(
  chunks: AnyIterable<AnyIterable<T>>,
  { maxFiles, comparer, store }: Omit<Options<T>, "maxSize">
) {
  const aggregated = [];
...
export async function* sort<T>(iterable: AnyIterable<T>, options?: Options<T>) {
  const { maxSize, maxFiles, comparer, store } = options ?? defaultOptions<T>();
  try {
    const sortedChunks = getSortedChunks(iterable, {
      maxSize,
      comparer,
      store,
    });
    const biggerChunks = aggregateChunks(sortedChunks, {
      maxFiles,
      comparer,
      store,
    });
    ...
    const merged =
      iterables.length === 1 ? iterables[0] : merge(iterables, comparer);
    yield* merged;
  } finally {
    await store.dispose();
  }
